...
import {
  Plus, Building2, Calendar, Globe, ExternalLink,
  Briefcase, Clock, ChevronRight, MoreHorizontal
} from "lucide-react";
      ...
      <div className="group bg-card border border-border rounded-xl p-4 hover:border-primary/40 hover:shadow-md transition-all cursor-pointer space-y-3">
        ...
        <div className="flex items-center justify-between gap-2">
          <span className="text-xs text-muted-foreground flex items-center gap-1">
            ...
            {app.appliedAt ? timeAgo(app.appliedAt) : "Draft"}
          </span>
          {app.source && (
        ...
        {(app as any).salaryOffered && (
          <p className="text-xs text-emerald-600 font-medium">
            ${Number((app as any).salaryOffered).toLocaleString()} offered
          </p>
        )}
      </div>
    </Link>
